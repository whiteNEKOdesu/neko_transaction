...
import neko.transaction.ware.feign.member.UserRoleFeignService;
import neko.transaction.ware.feign.member.UserWeightFeignService;
import neko.transaction.ware.mapper.ApiAuthInfoMapper;
import neko.transaction.ware.service.ApiAuthInfoService;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import neko.transaction.ware.to.UserRoleTo;
import neko.transaction.ware.to.UserWeightTo;
import neko.transaction.ware.vo.UpdateApiAuthInfoVo;
...
public class ApiAuthInfoServiceImpl extends ServiceImpl<ApiAuthInfoMapper, ApiAuthInfo> implements ApiAuthInfoService {
    @Resource
    private UserRoleFeignService userRoleFeignService;
    ...
    public void synchronizeApiToDB() {
        ...
        String key = Constant.WARE_REDIS_PREFIX + "api_auth_infos";
        //删除权限缓存
    ...
    public void updateApiAuthInfo(UpdateApiAuthInfoVo vo) {
        ...
        if(roleId != null){
            //远程调用member微服务获取角色信息
            ResultObject<UserRoleTo> r = userRoleFeignService.roleInfoById(roleId);
            if(!r.getResponseCode().equals(200)){
                throw new MemberServiceException("member微服务远程调用错误");
            }
            UserRoleTo userRole = r.getResult();
            if(userRole == null){
        ...
        if(weightId != null){
            //远程调用member微服务获取权限信息
            ResultObject<UserWeightTo> r = userWeightFeignService.userWeightById(weightId);
            if(!r.getResponseCode().equals(200)){
                throw new MemberServiceException("member微服务远程调用错误");
            }
            //获取权限信息
            UserWeightTo userWeight = r.getResult();
            if(userWeight == null){
